...
def inception_v3_arg_scope(weight_decay=0.00004,stddev=0.1,batch_norm_var_collection='moving_vars'):
    ...
    'beta':None,'gamma':None,'moving_mean':[batch_norm_var_collection],'moving_variance':[batch_norm_var_collection],
    }
    }
...
def inception_v3_base(inputs,scope=None):
    ...
    with tf.variable_scope(scope,'InceptionV3',[inputs]):
        with slim.arg_scope([slim.conv2d,slim.max_pool2d,slim.avg_pool2d],stride=1,padding='VALID'):
            ...
            net=slim.max_pool2d(net,[3,3],stride=2,scope='max_pool_5a_3x3')
        ...
        with slim.arg_scope([slim.conv2d,slim.max_pool2d,slim.avg_pool2d],stride=1,padding='SAME'):
            ...
            with tf.variable_scope('Mixed_7b'):
                ...
                with tf.variable_scope('Branch_2'):
                    ...
                    branch_2=tf.concat([slim.conv2d(branch_2,384,[1,3],scope='conv2d_2c_1x3'),slim.conv2d(branch_2,384,[3,1],scope='conv2d_2c_3x1')],3)
                with tf.variable_scope('Branch_3'):
                    branch_3=slim.avg_pool2d(net,[3,3],scope='avgpool_3a_3x3')
                    branch_3=slim.conv2d(branch_3,192,[1,1],scope='conv2d_3b_1x1')
                net=tf.concat([branch_0,branch_1,branch_2,branch_3],3)
        ...
        with slim.arg_scope([slim.batch_norm,slim.dropout],is_training=is_training):
            ...
            with slim.arg_scope([slim.conv2d,slim.max_pool2d,slim.avg_pool2d],stride=1,padding='SAME'):
                ...
                with tf.variable_scope('AuxLogits'):
                    aux_logits=slim.avg_pool2d(aux_logits,[5,5],stride=3,padding='VALID',scope='avgpool_0a_5x5')
                    aux_logits=slim.conv2d(aux_logits,128,[1,1],scope='conv2d_0b_1x1')
                    aux_logits=slim.conv2d(aux_logits,768,[5,5],weights_initializer=trunc_normal(0.01),padding='VALID',scope='conv2d_0c_5x5')
                    aux_logits=slim.conv2d(aux_logits,num_classes,[1,1],normalizer_fn=None,weights_initializer=trunc_normal(0.001),scope='conv2d_0d_1x1')
                    if spatial_squeeze:
