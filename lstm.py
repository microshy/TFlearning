...
class PTBmodel(object):
    def __init__(self,is_training,config,input_):
        ...
        cell=tf.contrib.rnn.MutilRNNCell([attn_cell() for _ in range(config.num_layers)],state_is_tuple=True)

        self._initial_state=cell.zero_state(batch_size,tf.float32)

        with tf.device('/cpu:0'):
            embedding=tf.get_variable('embedding',[vocab_size,size],ftype=tf.float32)
...
raw_data=reader.ptb_raw_data('simple_examples/data/')
...
with tf.Graph().as_default():
    ...
    with tf.name_scope("Test"):
        ...
        with tf.variable_scope("Model",reuse=True,initializer=initializer):
            mvalid=PTBModel(is_training=False,config=eval_config,input_=test_input)
    ...
    with sv.managed_session() as session:
        for i in range(config.max_max_epoch):
            ...
            m.assign_lr(session,config.learning*lr_decay)
